...
describe("base sidebar RTL", () => {
  it("anchors the desktop container to the logical start edge for side=left", () => {
    ...
    expect(sidebarContainer.className).toContain("data-[side=left]:start-0");
  ...
  it("anchors the desktop container to the logical end edge for side=right", () => {
    ...
    expect(sidebarContainer.getAttribute("data-side")).toBe("right");
    expect(sidebarContainer.className).toContain("data-[side=right]:end-0");
    expect(physicalEdgeUtilities(sidebarContainer)).toEqual([]);
  });
  ...
  it.each(["left", "right"] as const)(
    ...
    async (side) => {
      ...
      const flipped = side === "left" ? "2.5rem" : "-2.5rem";
      for (const phase of ["starting", "ending"] as const) {
        expect(sheet.className).toContain(
          `rtl:data-[side=${side}]:data-${phase}-style:translate-x-[${flipped}]`,
        );
      }
