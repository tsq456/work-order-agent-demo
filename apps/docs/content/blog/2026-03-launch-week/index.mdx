...
```sh
npx expo install @assistant-ui/react-native @assistant-ui/ai-sdk
```
...
<Callout type="info" title="Update">
The `cloud` command is no longer part of the CLI, and `@assistant-ui/cloud-ai-sdk` has since been retired. Follow the [Cloud quickstart](/docs/cloud/quickstart) to add thread persistence today, and see [Migrate from Cloud AI SDK](/docs/cloud/migrate-cloud-ai-sdk) if you use `useCloudChat`.
</Callout>
...
<Callout type="info" title="Update">
`@assistant-ui/cloud-ai-sdk` has since been retired: `useChatRuntime` from `@assistant-ui/ai-sdk` replaces it. See [Migrate from Cloud AI SDK](/docs/cloud/migrate-cloud-ai-sdk).
</Callout>
