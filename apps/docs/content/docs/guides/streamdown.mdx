...
<AgentSetup product="guides/streamdown" />
...
Your `pre` and `code` components are honored the same way as in react-markdown once the code adapter is active: with `SyntaxHighlighter`, `CodeHeader`, `componentsByLanguage`, or `pre` and `code` supplied together, `code` renders inline code and `pre`/`code` wrap fenced blocks and are the `Pre`/`Code` your `SyntaxHighlighter` receives. `code` alone is passed straight to Streamdown and owns the whole block, as shown below. `pre` alone only affects raw `<pre>` HTML without a `code` child, since fenced blocks then render through Streamdown's own code block.
...
| `components` | `object` | - | Custom components including `SyntaxHighlighter`, `CodeHeader`, and the `pre`/`code` they render through |
...
- [Markdown](/elements/markdown-text) - Lightweight markdown with react-markdown
- [Syntax Highlighting](/elements/syntax-highlighter) - Add code highlighting to react-markdown
- [Mermaid](/elements/mermaid-diagram) - Render diagrams in react-markdown
