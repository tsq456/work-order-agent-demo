...
aui.message.submitFeedback({ type: "negative", comment: "Quoted the wrong date" });
...
| Thread         | `isRunning` (may be explicitly set by the runtime rather than derived from last-message status), `isLoading`, `isDisabled`, `isEmpty`, `messages`, `tasks`, `capabilities`, `suggestions` | Active conversation state and message history |
...
| Thread         | `append(message)`, `startRun(config)`, `resumeRun(config)`, `cancelRun()`, `reset()`, `export()`, `import(repository)`, `message(selector)`, `task(selector)`, `composer()`, `getState()` | Message handling and conversation control |
...
Use `useAuiState` to render the current value of something; use `useAuiEvent` to react to the moment something happens. Both are supported: many events mark a transition you could also observe in state, and subscribing to the event is the intended way to run a side effect at that transition.

The rule of thumb:

1. Rendering a value? → use `useAuiState`.
2. Are you the caller and want immediate feedback? → catch the rejection / read the return value.
3. Running a side effect when something happens? → use `useAuiEvent`.

#### Available Events

| Event                         | When It Fires                                                                |
| ----------------------------- | ---------------------------------------------------------------------------- |
| `threads.selectionChanged`    | The main thread selection changed. Payload carries `threadId` (newly selected) and `previousThreadId`. Does not fire for the initially selected thread on mount. |
| `thread.runStart` / `runEnd`  | A run started / ended on the thread (also visible as `isRunning` flipping)   |
| `thread.initialize`           | The thread transitioned from new to initialized, before the first message is added |
...
| `composer.send`               | The user sent the composer contents; `messageId` is set for edit-composer sends |
| `composer.attachmentAdd`      | An attachment was added to the composer                                      |
| `composer.attachmentAddError` | An `addAttachment()` call failed. Payload `reason` discriminates `no-adapter` / `not-accepted` / `adapter-error`. The event additionally surfaces a human-readable `message` (and the underlying `Error` instance via the low-level `runtime.unstable_on("attachmentAddError")` API; `useAuiEvent` payloads omit it because raw `Error` objects are not store-serializable). |

#### Deprecated Events

| Deprecated Event              | Use Instead                                                                  |
| ----------------------------- | ---------------------------------------------------------------------------- |
| `threadListItem.switchedTo`   | `threads.selectionChanged` — its `threadId` is the newly selected thread     |
| `threadListItem.switchedAway` | `threads.selectionChanged` — its `previousThreadId` is the thread switched away from |
...
A component that renders outside `AssistantRuntimeProvider` has no scopes at all. Move it under the provider or publish the client to it; see [Reading State Outside the Thread](/docs/guides/state-outside-the-thread).
