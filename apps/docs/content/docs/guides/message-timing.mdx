...
<Callout type="info">
  The [`MessageTiming`](/elements/message-timing) registry component provides a ready-made badge + popover UI. This guide covers the underlying `useMessageTiming()` hook for custom implementations and runtime-specific setup.
</Callout>
