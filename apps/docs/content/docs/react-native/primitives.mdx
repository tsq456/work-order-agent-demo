...
| `maintainVisibleContentPosition` | `FlatListProps["maintainVisibleContentPosition"]` | Keeps the first visible message anchored while content is inserted above it, such as older history loading in. Defaults to `{ minIndexForVisible: 0 }` |
...
`Pressable` for adding attachments. It is disabled while the composer cannot accept attachments. React Native has no file input, so the caller opens the platform picker (for example `expo-document-picker` or `expo-image-picker`) in `onPress` and passes the selection to `aui.composer.addAttachment`.
  ...
  onPress={async () => {
    ...
    for (const asset of result.assets) {
      ...
      await aui.composer.addAttachment({
        name: asset.fileName ?? `image.${asset.mimeType.split("/")[1]}`,
...
There are no separate edit primitives. Inside a message, the composer primitives bind to that message's edit composer, so the same `ComposerPrimitive.Root`, `Input`, `Send`, and `Cancel` build the edit UI. `ActionBarPrimitive.Edit` starts editing, and `message.composer.isEditing` tells you when to render it.
...
function EditComposer() {
  return (
    <MessagePrimitive.Root>
      <ComposerPrimitive.Root style={styles.editComposer}>
        <ComposerPrimitive.Input multiline autoFocus style={styles.input} />
...
`Pressable` buttons for submitting message feedback. The submitted one is marked for assistive technology: `aria-selected` on native, `aria-pressed` on the web.
...
`Pressable` that creates a new thread. While the new thread is the current one, it is marked for assistive technology: `accessibilityState.selected` on native, `aria-current` on the web.
...
`Pressable` that switches to the thread. The current thread's trigger is marked for assistive technology: `accessibilityState.selected` on native, `aria-current` on the web.
